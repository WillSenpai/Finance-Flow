...
const Accademia = () => {
  ...
  const { data: completedLessons } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data.map((d: any) => d.lesson_id as string);
    },
    enabled: !!user,
    staleTime: 60 * 1000,
  });
  ...
  return (
    <div className="pt-14 pb-4">
      ...
      >
        ...
        <p className="text-muted-foreground text-sm mt-1">Impara al tuo ritmo, senza fretta</p>
      </motion.div>
      ...
      <div className="mt-6 space-y-8">
        {groupedCourses.map((courseGroup, catIdx) => {
          ...
          return (
            ...
            >
              <h2 className="text-base font-semibold px-5 mb-1">{section.title}</h2>
              <p className="text-xs text-muted-foreground px-5 mb-1">{section.description}</p>
              <p className="text-[11px] text-muted-foreground px-5 mb-3">
