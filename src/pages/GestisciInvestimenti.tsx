...
const GestisciInvestimenti = () => {
  const { investimenti, setInvestimenti } = useUser();
  const navigate = useNavigate();

  const [items, setItems] = useState<Investimento[]>([...investimenti]);
  const [values, setValues] = useState<Record<string, string>>(
    Object.fromEntries(investimenti.map((i) => [i.nome, String(i.valore)]))
  );
  ...
  const totale = items.reduce((a, i) => a + (parseInt(values[i.nome] || "0") || 0), 0);

  const handleAdd = () => {
    const nome = newNome.trim();
    if (!nome) { toast.error("Inserisci un nome"); return; }
    if (items.some((i) => i.nome.toLowerCase() === nome.toLowerCase())) {
      toast.error("Esiste già un investimento con questo nome"); return;
    }
    const color = defaultColors[items.length % defaultColors.length];
    const newItem: Investimento = { nome, valore: 0, emoji: newEmoji || "💎", colore: color };
    setItems([...items, newItem]);
    setValues((v) => ({ ...v, [nome]: "0" }));
    setNewNome("");
    setNewEmoji("💎");
  ...
  const handleRemove = (nome: string) => {
    setItems(items.filter((i) => i.nome !== nome));
    setValues((v) => { const n = { ...v }; delete n[nome]; return n; });
    toast("Asset rimosso");
  };

  const handleSave = () => {
    setInvestimenti(items.map((i) => ({ ...i, valore: Math.max(0, parseInt(values[i.nome] || "0") || 0) })));
    navigate(-1);
    toast.success("Investimenti aggiornati ✅");
  };

  // Chart data
  const chartData = items
    .map((i) => ({ nome: i.nome, valore: parseInt(values[i.nome] || "0") || 0, colore: i.colore, emoji: i.emoji }))
    .filter((d) => d.valore > 0);
  ...
  return (
    ...
    >
      ...
      <h1 className="text-2xl font-semibold tracking-tight mb-2">I tuoi Investimenti 📈</h1>
      <p className="text-sm text-muted-foreground mb-6">Gestisci, aggiungi o rimuovi i tuoi asset</p>

      {/* Add New Investment */}
      <div className="bg-card border border-border/50 rounded-2xl p-4 mb-6">
        <p className="text-xs font-semibold text-muted-foreground mb-3">Nuovo Investimento</p>
        <div className="flex gap-2 mb-3">
          <Input
            placeholder="Nome (es. Bitcoin)"
            value={newNome}
      ...
      <div className="space-y-3">
        <AnimatePresence initial={false}>
          {items.map((inv) => {
            const val = parseInt(values[inv.nome] || "0") || 0;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          type="number"
                          value={values[inv.nome] || ""}
                          onChange={(e) => setValues((v) => ({ ...v, [inv.nome]: e.target.value }))}
      ...
      {chartData.length > 0 && (
        <motion.div
          ...
          className="bg-card border border-border/50 rounded-2xl p-5 mt-6"
        >
...
export default GestisciInvestimenti;
